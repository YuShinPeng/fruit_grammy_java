...
public class MemberServiceImpl implements MemberService {
	...
	public SignupResponse addMember(SignupRequest request) {
		...
		for (Member item : memberList) {
			...
			if (!Pattern.matches(patternEmail, item.getEmail())) {
				return new SignupResponse("無效的信箱格式");
			}
	...
	public SignupResponse setMember(SignupRequest request) {
		...
		if (!StringUtils.hasText(request.getAccount()) && !StringUtils.hasText(request.getPassword())
				&& !StringUtils.hasText(request.getConfirm_password()) && !StringUtils.hasText(request.getPhone())
				&& !StringUtils.hasText(request.getAddress())) {
			return new SignupResponse("請輸入修改項目");
		}

		// 正規表達式
		String patternPhone = "[0][9]\\d{8}";
		String patternPwd = "^(?=.*[0-9])(?=.*[a-zA-Z]).{6,}$";

		if (StringUtils.hasText(request.getPassword()) || StringUtils.hasText(request.getConfirm_password())) {

			if (!Pattern.matches(patternPwd, request.getPassword())) {
				return new SignupResponse("密碼錯誤（英數字相加需要大於6個字元）");
			}
		...
		return new SignupResponse(updateMember, "修改成功");
	}
